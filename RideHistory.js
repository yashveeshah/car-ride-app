...
export default class RideHistoryScreen extends Component {
  ...
  handleSearch = async (carId, email) => {
    carId = carId.toUpperCase().trim();
    this.setState({
      allTransactions: []
    });
    if (!carId) {
  ...
  fetchMoreTransactions = async (carId, email) => {
    carId = carId.toUpperCase().trim();
        ...
        <ListItem key={i} bottomDivider>
          ...
          <ListItem.Content>
            <ListItem.Title style={styles.title}>
              {`${item.car_type} ( ${item.car_id} )`}
            </ListItem.Title>
            <ListItem.Subtitle style={styles.subtitle}>
              {`This car is ${transactionType} by you.`}
            </ListItem.Subtitle>
